...
export class FavoriteOfferEffects {
  ...
  getFavoriteOffers$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(() =>
        this.favoriteOfferService.getFavorites().pipe(
          map((favoriteOffers) =>
            FavoriteActions.loadFavoriteOffersSuccess({ favoriteOffers }),
          ),
          catchError((err: HttpErrorResponse) =>
            of(
              FavoriteActions.loadFavoriteOffersFailure({ error: err.message }),
            ),
          ),
        ),
      ),
  ...
  changeFavoriteOfferStatus$ = createEffect(() =>
    this.actions$.pipe(
      ofType(FavoriteActions.changeFavoriteOfferStatus),
      switchMap(({ offerId, status }) =>
        this.favoriteOfferService.changeFavoriteStatus(offerId, status).pipe(
          map((favoriteOffer) =>
            FavoriteActions.changeFavoriteOfferStatusSuccess({
              favoriteOffer: adaptOfferToPreview(favoriteOffer),
            }),
          ),
          catchError((err: HttpErrorResponse) =>
            of(
              FavoriteActions.changeFavoriteOfferStatusFailure({
                error: err.message,
              }),
            ),
          ),
